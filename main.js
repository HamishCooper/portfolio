...
function attachMediaLoader(containerEl, mediaEl){
  // Skip loader if already ready
  const tag = (mediaEl.tagName||'').toUpperCase();
  const isImg = tag === 'IMG';
  const isReady = isImg ? (mediaEl.complete && mediaEl.naturalWidth > 0)
                        : (typeof mediaEl.readyState === 'number' && mediaEl.readyState >= 1);
  if (isReady){
  ...
  const tick = () => {
    if (done) return;
    p = Math.min(80, p + 1 + Math.random()*2);
    fill.style.width = p + '%';
  ...
  const finish = () => {
    ...
    setTimeout(()=>{
      bar.style.opacity = '0';
      setTimeout(()=>{ try{ bar.remove(); }catch(e){} }, 200);
    }, 100);
    mediaEl.style.opacity = '1';
    containerEl.classList.remove('loading');
    containerEl.style.background = 'transparent';
  ...
  if (isImg){
    if (mediaEl.complete && mediaEl.naturalWidth > 0) finish();
    else {
      mediaEl.addEventListener('load',  finish, { once:true });
      mediaEl.addEventListener('error', finish, { once:true });
    }
  } else {
    // Video: hero (controls=false) vs block (controls=true)
    const isBlock = !!mediaEl.controls;
    ...
    if (!isBlock){
      // HERO: reveal on metadata so autoplay isn't blocked; controls are disabled elsewhere
      if (mediaEl.readyState >= 1) finish();
      else {
      ...
      // BLOCK VIDEO: show a real first frame without autoplay
      if (mediaEl.readyState >= 2) finish();
      else {
        mediaEl.addEventListener('loadedmetadata', ()=>{ try{ mediaEl.currentTime = 0; }catch(e){} }, { once:true });
        mediaEl.addEventListener('seeked',    finish, { once:true });
        mediaEl.addEventListener('loadeddata',finish, { once:true });
        mediaEl.addEventListener('canplay',   finish, { once:true });
        mediaEl.addEventListener('error',     finish, { once:true });
        mediaEl.addEventListener('progress',  tryBuffered);
        setTimeout(()=>{ if (!done && mediaEl.readyState >= 1) finish(); }, 1200);
      }
    }
  }
}let savedScrollY = 0;
...
if (cfg.noShadow) card.classList.add('no-shadow');
if (cfg.noInnerShadow) wrap.classList.add('no-inner');
...
function sizeHero(){
  ...
  let mediaW = Math.max(1, frame.clientWidth || 201);
  let mediaH = 0;
  if (el.tagName === 'IMG'){
...
// Optional grid layout via cfg.layout === 'grid' (or if any block has a span)
const wantsGrid = cfg.layout === 'grid' || (Array.isArray(cfg.blocks) && cfg.blocks.some(b => b && b.span));
if (wantsGrid){ infoBlocks.classList.add('blocks-grid'); } else { infoBlocks.classList.remove('blocks-grid'); }

(cfg.blocks || []).forEach(b => {
  const span = Math.max(1, Math.min(12, b.span || (wantsGrid ? 6 : 12)));
  const block = document.createElement('div');
  block.className = 'block' + (wantsGrid ? (' span-' + span) : '');
...
function updateOverlayHeight(){
  // Keep background static unless a detail page is open
...
function openDetail(cfg){
  ...
  // Immediately mark opening so credentials can transition offscreen
  document.body.classList.add('detail-opening');
  // Force a reflow so the credentials transform/opacity apply right away
  void document.body.offsetWidth;
// Immediate credentials transition without waiting for grid fade
  try{ document.body.classList.add('detail-opening'); }catch(_){}
  // Fade grid smoothly; then switch to detail-open (which hides grid) after a short delay
  try{
    grid.style.transition = grid.style.transition || 'opacity .14s ease';
    /*__GRID_FADE_WRAPPED__*/requestAnimationFrame(()=>{ try{ grid.style.opacity = '0'; }catch(_){}});
  }catch(_){}
// Fade grid out smoothly before entering detail-open (works on mobile & desktop)
  try{
    grid.style.transition = grid.style.transition || 'opacity .14s ease';
    /*__GRID_FADE_WRAPPED__*/requestAnimationFrame(()=>{ try{ grid.style.opacity = '0'; }catch(_){}});
  }catch(_){}
document.body.classList.remove('me-open');
  ...
  setTimeout(()=>{ document.body.classList.add('detail-open'); document.body.classList.remove('detail-opening'); }, 140);
...
function closeDetail(){
  ...
  // --- Fade grid back in after leaving detail ---
  try{
    // ensure a transition exists
    grid.style.transition = grid.style.transition || 'opacity .14s ease';
    // start from fully transparent (even if currently display:none)
    grid.style.opacity = '0';
  }catch(_){}
  // Remove any 'opening' state right away (credentials should come back normally)
  document.body.classList.remove('detail-opening');
  // Drop detail-open so grid becomes displayable again
  document.body.classList.remove('detail-open');
  // On next frame, commit opacity 1 so the fade actually runs
...
// Prepare grid to fade in
  try{
    grid.style.transition = grid.style.transition || 'opacity .14s ease';
    /*__GRID_FADE_WRAPPED__*/requestAnimationFrame(()=>{ try{ grid.style.opacity = '0'; }catch(_){}});
  }catch(_){}
  // Remove opening/open states immediately controlling credentials
  document.body.classList.remove('detail-opening');
  // Remove detail-open (grid becomes displayable again), then fade to 1 next frame
  requestAnimationFrame(()=>{
    try{
      void grid.offsetWidth; // force reflow so transition triggers
      grid.style.opacity = '1';
    }catch(_){}
  });
// Smoothly restore grid visibility
  try{
    grid.style.transition = grid.style.transition || 'opacity .14s ease';
    /*__GRID_FADE_WRAPPED__*/requestAnimationFrame(()=>{ try{ grid.style.opacity = '0'; }catch(_){}});
  }catch(_){}
  ...
  const doShowGrid = () => {
    // Remove detail-open first so grid is displayed again
...
document.body.classList.remove('me-open');
    document.dispatchEvent(new Event('cred:force'));
  ...
  /*__FINAL_GRID_SHOW__*/
  requestAnimationFrame(()=>{ try{ void grid.offsetWidth; grid.style.opacity = '1'; }catch(_){ } });
...
function buildGridMobile(data) {
  ...
  // Fisher-Yates shuffle — uniform, stable, efficient
  for (let i = shuffled.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]];
  }

  shuffled.forEach(cfg => grid.appendChild(createCard(cfg)));
}
...
// === ME page support (minimal; non-intrusive) ===
function resolveMeConfig(){
  try {
    if (Array.isArray(window.meConfig) && window.meConfig.length) return window.meConfig[0];
    if (typeof meConfig !== 'undefined'){ return Array.isArray(meConfig) ? meConfig[0] : meConfig; }
  } catch(e){}
  return null;
}
function fillMe(){
  const cfg = resolveMeConfig();
  if (cfg) fillInfo(cfg);
}
function openMe(){
if (document.body.classList.contains('me-open')) return;

  // Dim PORTFOLIO immediately
  try{ const pl = document.querySelector('#credentials .portfolio .portfolio-link'); if (pl) pl.style.opacity = '0.4'; }catch(_){}

  // Enable scoped animation helpers
  document.body.classList.add('me-anim');

  // Fade grid to 0 over 0.3s
  grid.style.opacity = getComputedStyle(grid).opacity || '1';
  requestAnimationFrame(()=>{ grid.style.opacity = '0'; });

  setTimeout(()=>{
    // Switch to ME layout after grid fade completes
    document.body.classList.add('me-open');
    document.body.classList.remove('base');

    // Render ME and fade it in
    try{ fillMe && fillMe(); }catch(_){}
    infoPanel.classList.add('visible');
    infoPanel.style.opacity = '0';
    requestAnimationFrame(()=>{ infoPanel.style.opacity = '1'; });

    // Cleanup after ME fade-in
    setTimeout(()=>{
      infoPanel.style.opacity = '';
      document.body.classList.remove('me-anim');
      requestAnimationFrame(updateOverlayHeight);
      window.addEventListener('resize', updateOverlayHeight);
      window.addEventListener('scroll', updateOverlayHeight);
    }, 300);
...
if (!document.body.classList.contains('me-open')) return;
  ...
  try{ const pl = document.querySelector('#credentials .portfolio .portfolio-link'); if (pl) pl.style.opacity = '1'; }catch(_){}

  // Enable scoped animation helpers
  document.body.classList.add('me-anim');

  // Fade ME panel out
  infoPanel.style.opacity = getComputedStyle(infoPanel).opacity || '1';
  requestAnimationFrame(()=>{ infoPanel.style.opacity = '0'; });

  setTimeout(()=>{
    infoPanel.classList.remove('visible');
    infoPanel.style.opacity = '';

    // Switch layout, then fade grid back in
    document.body.classList.remove('me-open');
    document.dispatchEvent(new Event('cred:force'));
    document.body.classList.add('base');
...
(function wireMeLinks(){
  const cred = document.getElementById('credentials');
  if (!cred) return;
  const port = cred.querySelector('.portfolio');
  const me   = cred.querySelector('.me-link');
  if (port){
    port.style.cursor = 'pointer';
    port.addEventListener('click', function(e){
      e.preventDefault();
      
      if (!e.__credAnimated){ e.__credAnimated = true;
        try{
          const cred = document.getElementById('credentials');
          const meL  = cred && (cred.querySelector('.me .me-link') || cred.querySelector('.me-link'));
          const plL  = cred && cred.querySelector('.portfolio .portfolio-link');
          // Clicking PORTFOLIO — from ME page we want: ME -> 0.4 in 1s, PORTFOLIO -> 1 in 0.2s
          const onME = document.body.classList.contains('me-open');
          if (onME){
            if (plL){ plL.style.transition = 'opacity 0.2s ease'; plL.style.opacity = '1'; }
            if (meL){ meL.style.transition = 'opacity 1s ease';  meL.style.opacity = '0.4'; }
          } else {
            // From grid, PORTFOLIO already 1 and ME 0.4 by CSS; still ensure durations for consistency
            if (plL){ plL.style.transition = 'opacity 0.2s ease'; }
            if (meL){ meL.style.transition = 'opacity 1s ease'; }
          }
          setTimeout(()=>{ if (plL) plL.style.transition=''; if (meL) meL.style.transition=''; }, 1200);
        }catch(_){}
      }
if (document.body.classList.contains('me-open')) closeMe();
  ...
  if (me){
    me.addEventListener('click', function(e){
      e.preventDefault();
      // Immediately move credentials to x=0 on ME click (smooth via existing CSS)
      try{ const el = document.getElementById('credentials'); if (el){ el.style.transform=''; } }catch(_){}
      ...
      if (!e.__credAnimated){ e.__credAnimated = true;
        try{
          ...
          const meL  = cred && (cred.querySelector('.me .me-link') || cred.querySelector('.me-link'));
          const plL  = cred && cred.querySelector('.portfolio .portfolio-link');
          // Clicking ME — from grid we want: ME -> 1 in 0.2s, PORTFOLIO -> 0.4 in 1s
          const onME = document.body.classList.contains('me-open');
          if (!onME){
            if (meL){ meL.style.transition = 'opacity 0.2s ease'; meL.style.opacity = '1'; }
            if (plL){ plL.style.transition = 'opacity 1s ease';  plL.style.opacity = '0.4'; }
          } else {
            // Already on ME; ensure durations for consistency
            if (meL){ meL.style.transition = 'opacity 0.2s ease'; }
            if (plL){ plL.style.transition = 'opacity 1s ease'; }
          }
          setTimeout(()=>{ if (meL) meL.style.transition=''; if (plL) plL.style.transition=''; }, 1200);
        }catch(_){}
      }
openMe();
    }, { passive:false });
...
;
// Ensure opening a detail card exits ME mode
(function(){
  const _openDetail = openDetail;
  openDetail = function(cfg){
    document.body.classList.remove('me-open');
    _openDetail(cfg);
  };
})();
...
(()=>{
  ...
  // === Credentials follow grid horizontally after 240px on desktop ===
  const CRED_PIN_PX = 240;
  let credEl = null;
  let credScroller = null;
  function ensureCredEl(){
    if (!credEl) credEl = document.getElementById('credentials');
    return credEl;
  }
  function syncCredentialsWithScroll(){
    const el = ensureCredEl();
    if (!el) return;
    if (!isDesktop() || inDetail() || document.body.classList.contains('me-open')){
      el.style.transform = '';
      return;
    }
    ...
    const s = scroller;
    if (!s) return;
    const x = s.scrollLeft || 0;
    const delta = Math.max(0, x - CRED_PIN_PX);
    if (delta > 0){
  ...
  // Immediate credentials transform that ignores 'me-open' (used on PORTFOLIO click)
  function __credImmediateApply(){
    try{
      const el = ensureCredEl(); if (!el) return;
      if (!scroller) scroller = pickScroller();
  ...
  document.addEventListener('cred:force', __credImmediateApply, { passive:true });

  function onCredScrollerScroll(){ syncCredentialsWithScroll(); }
  function attachCredScroller(){
    if (!isDesktop()) return;
    if (!scroller) scroller = pickScroller();
    if (!scroller) return;
    if (credScroller !== scroller){
      if (credScroller){
        try{ credScroller.removeEventListener('scroll', onCredScrollerScroll); }catch(_){}
      }
      credScroller = scroller;
      credScroller.addEventListener('scroll', onCredScrollerScroll, { passive:true });
    }
    syncCredentialsWithScroll();
  }
function isDesktop(){ return window.matchMedia(DESKTOP_Q).matches; }
  ...
  function onWheel(e){ if (!isDesktop() || inDetail() || document.body.classList.contains("me-open")) return;
    ...
    try{ syncCredentialsWithScroll(); }catch(_){}
  }
  ...
  function init(){
    ...
    attachCredScroller();
    syncCredentialsWithScroll();
    window.addEventListener('resize', () => { scroller = null; credScroller = null; syncCredentialsWithScroll(); attachCredScroller(); }, { passive:true });
  }
...
/* === Credentials: 'PORTFOLIO' link closes ME panel === */
(function(){
  const plink = document.querySelector('#credentials .portfolio .portfolio-link');
  if (!plink) return;
  plink.addEventListener('click', function(e){
    e.preventDefault();
    e.stopImmediatePropagation(); if (!e.__credAnimated){ e.__credAnimated = true;
      try{
        const cred = document.getElementById('credentials');
        const meL  = cred && (cred.querySelector('.me .me-link') || cred.querySelector('.me-link'));
        const plL  = cred && cred.querySelector('.portfolio .portfolio-link');
...
// [removed legacy ME Page toggler]
